#!/bin/bash
# 模型灾备脚本 v5 - 快速失败 + 云端优先本地回退
# 核心改进：
# 1. 云端失败 → 立即切换本地（不尝试其他云端）
# 2. log() 输出到 stderr，不污染返回值
# 3. 严格的配置验证，防止写入非法值
...
find_available_model() {
    ...
    if [ -n "$primary" ] && [ "$primary" != "null" ]; then
        ...
        if [[ "$primary" == *":cloud"* ]]; then
            # 云端模型：失败立即切换本地
            if test_model_api "$primary"; then
                log_ok "当前云端模型可用"
                return 0
            fi
            log "云端不可用，立即切换本地" >&2
    ...
    for cloud_model in $cloud_models; do
        if try_cloud_model "ollama/$cloud_model"; then
